package com.unir.relatospapel.msgateway.fllter;

import com.unir.relatospapel.msgateway.decorator.RequestDecoratorFactory;
import com.unir.relatospapel.msgateway.model.GatewayRequest;
import com.unir.relatospapel.msgateway.utils.RequestBodyExtractor;
...
public class RequestTranslationFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(
        ...
        if (exchange.getRequest().getHeaders().getContentType() == null || !exchange.getRequest().getMethod().equals(HttpMethod.POST)) {
            log.error("Request does not have a content type or is not a POST request");
